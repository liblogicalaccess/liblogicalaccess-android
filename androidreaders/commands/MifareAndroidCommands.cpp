...
MifareAndroidCommands::readBlock(int sector, int block, std::vector<unsigned char> keyA)
{
    ...
    jmethodID execCommand =
        env->GetStaticMethodID(jNDKhelper, "mifareReadBlock", "(II[B)[B");
    jbyteArray resultDataArray = (jbyteArray)env->CallStaticObjectMethod(
        jNDKhelper, execCommand, sector, block, array);
